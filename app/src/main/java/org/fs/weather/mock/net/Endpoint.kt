...
import org.fs.weather.util.C.Companion.QUERY_SEARCH
...
interface Endpoint {
  ...
  @GET(WEATHER_REQUEST_PATH) fun weatherFor(@Query(QUERY_SEARCH) q: String, @Query(QUERY_NUMBER_OF_DAYS) days: Int): Observable<Response<Forecast>>

  @GET(SEARCH_REQUEST_PATH) fun cityFor(@Query(QUERY_SEARCH) q: String): Observable<Response<SearchResponse<List<City>>>>
}