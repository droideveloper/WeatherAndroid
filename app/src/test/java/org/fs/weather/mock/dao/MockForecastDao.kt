...
class MockForecastDao(private val dataSet: ArrayList<Forecast>): ForecastDao {

  override fun forecastByCityId(cityId: Long): Single<Forecast> = Single.just(dataSet.firstOrNull { f -> f.cityId == cityId })

  override fun create(forecast: Forecast) {
    dataSet.add(forecast)
  }

  override fun update(forecast: Forecast) {
    val index = dataSet.indexOfFirst { f -> f.cityId == forecast.cityId }
    if (index != -1) {
      dataSet[index] = forecast
    }
  }
